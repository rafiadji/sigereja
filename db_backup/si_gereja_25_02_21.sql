...
-- Records of nikah
-- ----------------------------
INSERT INTO `nikah` VALUES (1, 139, 'Maria', 'Malang', 'Sukijan', NULL);
...
INSERT INTO `user` VALUES (139, '82630004', 82630001, 'Sinu', 'Laki-laki', 'Malang', '2019-06-19', 'Maria', 'Terdaftar', 'Terdaftar', 'Anak', 'Swasta', '', '', '', 0, 0, 0);
...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `daftar_baptis` AS select `baptis`.`baptis_id` AS `baptis_id`,`user`.`user_id` AS `user_id`,`user`.`nik` AS `nik`,`user`.`nik_kk` AS `nik_kk`,`user`.`name` AS `name`,`baptis`.`kat_baptis` AS `kat_baptis`,`baptis`.`nama_baptis` AS `nama_baptis`,`baptis`.`tanggal_baptis` AS `tanggal_baptis`,`baptis`.`tempat_baptis` AS `tempat_baptis`,`user`.`st_baptis` AS `st_baptis` from (`baptis` join `user` on((`baptis`.`user_id` = `user`.`user_id`)));

-- ----------------------------
-- View structure for daftar_nikah
-- ----------------------------
DROP VIEW IF EXISTS `daftar_nikah`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `daftar_nikah` AS select `nikah`.`nikah_id` AS `nikah_id`,`user`.`user_id` AS `user_id`,`user`.`nik` AS `nik`,`user`.`nik_kk` AS `nik_kk`,`user`.`name` AS `name`,`nikah`.`n_pasangan` AS `n_pasangan`,`nikah`.`a_pasangan` AS `a_pasangan`,`nikah`.`n_saksi` AS `n_saksi`,`nikah`.`tgl_nikah` AS `tgl_nikah`,`user`.`st_nikah` AS `st_nikah` from (`nikah` join `user` on((`nikah`.`user_id` = `user`.`user_id`)));

-- ----------------------------
-- Triggers structure for table baptis
-- ----------------------------
DROP TRIGGER IF EXISTS `update_st_baptis`;
delimiter ;;
CREATE TRIGGER `update_st_baptis` AFTER INSERT ON `baptis` FOR EACH ROW BEGIN
...
-- ----------------------------
DROP TRIGGER IF EXISTS `update_st_nikah`;
delimiter ;;
CREATE TRIGGER `update_st_nikah` AFTER INSERT ON `nikah` FOR EACH ROW BEGIN
